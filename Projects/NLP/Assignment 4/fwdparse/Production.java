...
public class Production
{
	...
	public int getChildSize(){
		...
		/*System.out.println("Count = "+count);
				if (children!=null){
					...
					for(int i = 0; i< children.length; i++){
						try{
							//count+= children[i].getChildSize()+(children[i].start-this.start);
							System.out.println("Count = "+count);
							//System.out.println("\t"+children[i].left+" = "+count+", ");
						}
						catch(NullPointerException e){
							System.out.println("\t"+children[i].left+" = Null, ");
						}
					}
				}
				else{
					System.out.println(this.left+" = empty");
					count++;
				}*/
				for(int i =0; i<right.length; i++){
	...
	{
		//Debug
		//System.out.print("recursivePrint invoked!");
		//System.out.println(this.toString());
		System.out.print(left.toUpperCase()+" [ ");
		for(int i =0; i<right.length; i++){
			if(this.children!=null){
				//Debug
				//System.out.println("Recursing "+this.children[i]+"\n");
				this.children[i].recursivePrint();
			}
			else{
				for(int j=0; j< right.length; j++){
					//Debug
					//System.out.println("NULL Child! so the right is ");
					System.out.print(right[j]);
				}
			}
		}
